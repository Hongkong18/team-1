(function ($) {
  ...
  var week_number = 0;
  var curr = new Date;
  var sun = new Date(curr.setDate(curr.getDate() - curr.getDay())); // First day is the day of the month - the day of the week

  function formatDate(value) {
   return (value.getYear()+1900) + "-" + (value.getMonth()+1) + "-" + (value.getDate()) ;
  }
  // var firstday = formatDate(new Date(curr.setDate(first+5+week_number))).toString();
  ...
  DayScheduleSelector.prototype.renderRows = function () {
    ...
    $.each(generateDates(start, end, interval), function (i, d) {
      var daysInARow = $.map(new Array(days.length), function (_, i) {
        return '<td class="time-slot" data-time="' + hhmm(d) + '" data-day="' +(sun.getYear()+1900).toString()+"-"+(sun.getMonth()+1).toString()+"-"+(sun.getDate()+days[i]).toString() + '" data-weekday="' + days[i] + '"></td>';
      }).join();
  ...
  DayScheduleSelector.prototype.attachEvents = function () {
    ...
    this.$el.on('click', '.time-slot', function () {
      ...
      var time = new Date(2017,10,25,15,0,0,0);
      // if (!plugin.isSelecting()) {  // if we are not in selecting mode
      //   if (isSlotSelected($(this))) { plugin.deselect($(this)); }
      //   else {  // then start segit lecting
      var start = this.getAttribute("data-time");

        var SelectedHour = parseInt(start.charAt(0) + start.charAt(1));

        var currentHour = parseInt(time.getHours());
        var currentDay = parseInt(time.getDay()) ;
        ...
        console.log(day);
          if (((SelectedHour > currentHour) && (day != currentDay)) ||  ((SelectedHour < currentHour) && (day != (currentDay+1)))  ){
         ...
         //
         //   $(this).attr('data-selecting', 'selected').removeAttr('data-selecting');
         //
         // }
